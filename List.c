...
bool equals(List A, List B) // Returns true iff Lists A and B are in same state, and returns false otherwise.
{
   if(A == NULL)
   {
      fprintf(stderr, "Error: List A is Null\n");
      exit(EXIT_FAILURE);
   }
   
   if(B == NULL)
   {
      fprintf(stderr, "Error: List B is Null\n");
      exit(EXIT_FAILURE);
   }
...
List copyList(List L)// Returns a new List representing the same integer sequence as L. The cursor in the new list is undefined
{
   if(L == NULL)
   {
      fprintf(stderr, "Error: List is Null\n");
